...
func TestStatefulsetScheduler(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name:      "two replicas, 22 vreplicas, already scheduled on overcommitted pod, remove placement",
			vreplicas: 22,
			replicas:  int32(4),
			placements: []duckv1alpha1.Placement{
				{PodName: "statefulset-name-0", VReplicas: 1},
		...
		{
			name:      "two replicas, 24 vreplicas, already scheduled on overcommitted pod, remove placement",
			vreplicas: 24,
			replicas:  int32(11),
			placements: []duckv1alpha1.Placement{
				{PodName: "statefulset-name-0", VReplicas: 1},
				{PodName: "statefulset-name-1", VReplicas: 5},
				{PodName: "statefulset-name-10", VReplicas: 5},
			},
			additionalVPods: []VPod{
				{
					VReplicas: 4,
				...
				},
				{
					VReplicas: 5,
					Placements: []duckv1alpha1.Placement{
						{PodName: "statefulset-name-3", VReplicas: 5},
					},
				},
				{
					VReplicas: 10,
					Placements: []duckv1alpha1.Placement{
						{PodName: "statefulset-name-4", VReplicas: 10},
					},
				},
				{
					VReplicas: 5,
					Placements: []duckv1alpha1.Placement{
						{PodName: "statefulset-name-10", VReplicas: 4},
					},
				},
			},
			expected: []duckv1alpha1.Placement{
				...
				{PodName: "statefulset-name-2", VReplicas: 10},
				{PodName: "statefulset-name-3", VReplicas: 2},
				{PodName: "statefulset-name-5", VReplicas: 2},
				{PodName: "statefulset-name-10", VReplicas: 6},
			},
		},
		{
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			for i, v := range tc.additionalVPods {
				vpodClient.Create(vpodNamespace, fmt.Sprintf("%s-additional-%d", vpodName, i), v.VReplicas, v.Placements)
			}
