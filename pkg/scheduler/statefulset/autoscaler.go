...
func (a *autoscaler) doautoscale(ctx context.Context, attemptScaleDown bool) error {
	...
	expectedReplicas := int32(math.Ceil(float64(state.TotalExpectedVReplicas()) / float64(state.Capacity)))

	// Only scale down if permitted
	if expectedReplicas < scale.Spec.Replicas && !attemptScaleDown {
		return nil
	}

	if expectedReplicas != scale.Spec.Replicas {
		scale.Spec.Replicas = expectedReplicas
