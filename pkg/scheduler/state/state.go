...
func (s *stateBuilder) State(ctx context.Context) (*State, error) {
	...
	state := &State{FreeCap: free, SchedulablePods: schedulablePods.List(), LastOrdinal: last, Capacity: s.capacity, Replicas: scale.Spec.Replicas,
		StatefulSetName: s.statefulSetName, PodLister: s.podLister,
		PodSpread: podSpread, ExpectedVReplicaByVPod: expectedVReplicasByVPod}
